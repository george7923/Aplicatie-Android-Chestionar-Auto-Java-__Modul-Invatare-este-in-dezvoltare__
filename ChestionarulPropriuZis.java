...
public class ChestionarulPropriuZis extends AppCompatActivity {
    ...
    private void FormareIntrebari() {
        ...
        if (I.isEmpty()) {
            ...
            for (int i = 0; i < 26; i++) {
                I26.add(getRandomQuestion(I));
            }
        }
    }

    private Intrebare getRandomQuestion(ArrayList<Intrebare> I) {
        if (I.isEmpty()) {
    ...
    private void GestorChestionar(Intrebare Q) {
        ...
        INTREBAREA.setTextColor(Color.WHITE); // Setează culoarea textului la alb

        CheckBox A = findViewById(R.id.R_A);
        A.setText(Q.getRaspuns1().getText());
        A.setTextColor(Color.WHITE); // Setează culoarea textului la alb

        CheckBox B = findViewById(R.id.R_B);
        B.setText(Q.getRaspuns2().getText());
        B.setTextColor(Color.WHITE); // Setează culoarea textului la alb

        CheckBox C = findViewById(R.id.R_C);
        C.setText(Q.getRaspuns3().getText());
        C.setTextColor(Color.WHITE); // Setează culoarea textului la alb
    ...
    public void countdownTimer() {
        new CountDownTimer(timpRamas, 1000) { // milisecunde până la final, interval între actualizări
            ...
            public void onFinish() {
                timpRamas = 0;
                updateTimerText();
                // Aici poți face ceva atunci când timpul se termină
            }
        }.start();
    }
